...
class CareProvider(models.Model):
    careprovider_username = (
        models.CharField(max_length=200, unique=True, primary_key=True)
        )
    ...
    type_of_care = (
        models.CharField(
            max_length=25, choices=SPECIALITY_CHOICES, default="ELDERLY CARE")
        )
    main_contact_name = models.CharField(max_length=100)
    address_line_1 = models.CharField(max_length=250)
    county = (
        models.CharField(
            max_length=9, choices=COUNTY_CHOICES, default="DUBLIN")
        )
    ...
    provider_approved_status = (models.IntegerField(
        choices=PROVIDER_APPROVED_STATUS, default=0)
        )
    likes = models.ManyToManyField(
        User, related_name='provider_likes', blank=True)
...
class CareProviderComments(models.Model):
    ...
    careprovider = (models.ForeignKey(
        CareProvider, on_delete=models.CASCADE,
